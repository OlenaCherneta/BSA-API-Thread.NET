...
describe("4. Create a post", () => {
        ...
        expect(response.body, "JSON Schema failed").to.be.jsonSchema(schemas.schema_onePost);
        checkResponseTime(response, 1000);
        expect(response.body.body, `Post body is the same as sent`).to.be.equal(body);
...
describe("6. Add comment to the post", () => {
    it(`Adding comment to the post`, async () => {
        ...
        expect(response.statusCode, `Status Code should be 200`).to.be.equal(200);
        checkResponseTime(response, 1000);
        expect(response.body, "JSON Schema failed").to.be.jsonSchema(schemas.schema_oneComment);
